...
const rows = lessons.flatMap(lesson => lesson.infinite ? [] : lesson.exercises.map((exercise, index) => `(${literal(lesson.id)},${index},${literal(lesson.stage)},${literal(normalize(exercise))},'1')`))
